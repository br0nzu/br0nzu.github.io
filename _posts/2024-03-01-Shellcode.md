...
## Shellcraft()
위의 방법들처럼 직접 쉘코드를 작성할 수 있지만, pwntools의 기능인 `shellcraft()`를 이용하여 손쉽게 쉘코드를 작성할 수 있다.
...
context.arch = "amd64" # x86환경이면 "i386"
context.os = "linux"
...
shellcode += shellcraft.open("rsp", 0, 0)       # rsp -> esp
# read(fd, buf, 100)
shellcode += shellcraft.read("rax", "rsp", 100) # rax -> eax / rsp -> esp
# write(1, buf, 100) 
shellcode += shellcraft.write(1, "rsp", 100)    # rsp -> esp
```
...
[3] [Ascii shellcode](https://nets.ec/Ascii_shellcode)