...
#### 대칭키 암호기술
**대칭키 암호기술**은 암호화 키와 복호화 키가 동일한 암호기술이다. 대칭키 암호기술에는 **스트림 암호**와 **블록 암호**가 있다. **스트림 암호**는 데이터를 연속적인 비트나 바이트 단위로 암호화하며, 주로 실시간 데이터 암호화에 사용된다. 스트림 암호는 기밀성을 제공하며, 대표적인 예로 RC4가 있다. **블록 암호**는 데이터를 고정된 크기의 블록 단위로 암호화한다. 블록 암호는 기밀성과 더불어 인증 기능도 제공할 수 있다. 대표적인 블록 암호로는 DES와 AES가 있다.
